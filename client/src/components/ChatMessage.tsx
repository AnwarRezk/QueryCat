...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  return (
    <>
      ...
      >
        <div className={`flex max-w-[95%] md:max-w-[90%] lg:max-w-[85%] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
          
          {/* Avatar */}
          <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mt-1 
            ${isUser ? 'bg-gradient-glow ml-2 md:ml-3' : 'glass-panel border-white/10 mr-2 md:mr-3'}`}
          >
          ...
          <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
            <div className={`py-2.5 px-3 md:py-3 md:px-4 ${isUser 
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-left">
                ...
                {displayedSources.map((src, i) => (
                  <div key={i} className="flex items-center space-x-1 glass-panel px-2 py-1 rounded-md border-white/5 bg-surface-hover">
                    ...
                    <span className="text-xs text-gray-300 truncate max-w-[120px] sm:max-w-[200px]">{src.source} (p. {src.page})</span>
                  </div>
                ))}
